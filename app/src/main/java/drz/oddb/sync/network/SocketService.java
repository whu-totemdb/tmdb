...
public class SocketService {
    ...
    private byte[] getDataToTransport(GossipRequest request){
        ...
        System.out.println("节点" + request.getSourceIPAddress().toString() + "正在向缓存区写入待传输的数据");
        try{
            ...
            System.out.println("由于" + e.getMessage() + "节点" + request.getSourceIPAddress().toString() + "发送失败...");
            e.printStackTrace();
        }
    ...
    private void sendToTargetNode(InetAddress source, InetSocketAddress target , byte[] data) throws IOException{
        ...
        try{
            System.out.println("节点"+ source.toString() + "向节点" + target.toString() + "发送数据");
            sendDatagramSocket.send(packet);
        }catch (IOException e){
            System.out.println("节点"+ source.toString() + "向节点" + target.toString() + "发送数据失败");
            e.printStackTrace();
        }
    }
