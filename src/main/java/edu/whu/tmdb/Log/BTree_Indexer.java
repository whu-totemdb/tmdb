...
public class BTree_Indexer<K, V> {
    ...
    private static class BTreeNode<K, V> {
        ...
        public BTreeNode(String fileName, long offset){
            ...
            // 首先读4字节判断节点的type
            int type = Constant.BYTES_TO_INT(Constants.readBytesFromFile( offset, 4, fileName), 0, 4);
            offset += 4;
            ...
            if(type == 1){
                ...
                int entryCount = Constant.BYTES_TO_INT(Constants.readBytesFromFile( offset, 4, fileName), 0, 4);
                offset += 4;
                // 每个Entry即每个k-v占用的字节数
                ...
                // 读取entryCount个Entry
                byte[] buffer = Constants.readBytesFromFile(offset, totalLength, fileName);
                offset += totalLength;
                // 构造entryCount个Entry
            ...
            // 非叶子节点
                ...
                // 读4字节即一个int，得到有多少个entry
                int entryCount = Constant.BYTES_TO_INT(Constants.readBytesFromFile( entryStartOffset, 4, fileName), 0, 4);
                // 每个Entry即每个k-v占用的字节数
                int singleEntryLength = Constant.MAX_KEY_LENGTH + Long.BYTES;
                // Entry总占用字节数
                ...
                int childrenCount = Constant.BYTES_TO_INT(Constants.readBytesFromFile( childrenStartOffset, 4, fileName), 0, 4);
                // 每个child占用一个long 8字节，共占用
                int totalChildrenLength = childrenCount * Long.BYTES;

                // 2.读取childrenCount个child
                byte[] buffer = Constants.readBytesFromFile(childrenStartOffset + 4, totalChildrenLength, fileName);
                ...
                // 读取entryCount个Entry
                buffer = Constants.readBytesFromFile(entryStartOffset + 4, totalEntryLength, fileName);
                // 构造entryCount个Entry
                for(int i=0; i<entryCount; i++){
                    byte[] b;
                    // K
        ...
        public long write(BufferedOutputStream outputStream, long offset) throws IOException {
            ...
            if(leaf){
                ...
                for(Entry entry : entrys){
                    // 写key
                    outputStream.write(entry.key.toString().getBytes());
